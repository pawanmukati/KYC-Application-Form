
module.exports = ({
investment_commercial_acc
,personal_corporate_acc
,signatory_acc
,signature_right
,bank_account_country
,bank_account_country_other_optional
,particular_bank
,firstname
,lastname
,dob
,nationality
,address
,CityOfResidence
,MaritalStatus
,email
,phone
,partner_spouse
,name_age_children
,father
,mother
,siblings
,study
,graduate
,profession
,cfc_studies
,cfc_studies_from_year
,cfc_studies_to_year
,cfc_studies_title
,actual_employer
,employer_title
,business_nature
,business_headquarter_country
,business_headquarter_city
,employment_start_date
,company_website
,share_of_company
,approximate_turnover_company
,approx_employees
,approx_value_of_company
,worked_before
,before_from_year
,before_to_year
,before_company_name
,before_work_title
,before_work_compensation_per_year
,political_exposed_person
,cfc_signature_right
,cfc_signature_right_people
,cfc_signature_right_name
,cfc_signature_right_relation
,cfc_signatories_beneficial_owners
,cfc_signatories_beneficial_owners_name
,cfc_signatories_beneficial_owners_relation
,amount_in_usd
,amount_in_usd_source
,financial_worth_in_usd
,assets_type
,assets_country
,assets_city
,assets_approx_value
,name_of_bank
,bankable_assets_approx_amount
,source_of_wealth
,source_of_wealth_amount
,investment_objective
,risk_tolerance
,investment_experience
,investment
,investment_knowledge
,cfc_open_corporate_account
,cfc_open_corporate_account_name_company
,cfc_open_corporate_account_jurisdiction_company
,cfc_open_corporate_account_type_company
,cfc_open_corporate_account_beneficial_owners
,cfc_open_corporate_account_beneficial_owners_firstname
,cfc_open_corporate_account_beneficial_owners_lastname
,cfc_open_corporate_account_beneficial_owners_nationality
,cfc_open_corporate_account_beneficial_owners_dob
,cfc_open_corporate_account_beneficial_owners_CityOfResidence
,cfc_open_corporate_account_beneficial_owners_Relation_each_other
,cfc_open_corporate_account_directors
,cfc_open_corporate_account_directors_firstname
,cfc_open_corporate_account_directors_lastname
,cfc_open_corporate_account_directors_nationality
,cfc_open_corporate_account_directors_dob
,cfc_open_corporate_account_directors_CityOfResidence
,cfc_open_corporate_account_directors_Relation_each_other
,cfc_open_corporate_account_settlor
,cfc_open_corporate_account_settlor_firstname
,cfc_open_corporate_account_settlor_lastname
,cfc_open_corporate_account_settlor_nationality
,cfc_open_corporate_account_settlor_dob
,cfc_open_corporate_account_settlor_CityOfResidence
,cfc_open_corporate_account_settlor_Relation_each_other
,approx_amount_transfer_new_bank_account
,send_fund_from_bank_name
,send_fund_from_bank_country
,transfers_intend_to_do_every_year
,transfers_intend_to_receive_every_year
,confirm_information_provided
,understand_information_provided
,confirm_email

    
})=>{
  const today = new Date()
  return `
  <html>
   <head>
      <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
      <title>Contact Form 7 Email Add on</title>
   </head>
   <body style="margin: 0; padding: 0;">
      <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#ffffff" border="0" style="background-image: url(https://www.clevermonks.com/wp-content/plugins/cf7-email-add-on/admin/assets/images/table-bg.png); /* background-color: #ffffff; */ padding-top: 20px;">
         <tbody>
            <tr>
               <td align="center" style="padding-left: 15px; padding-right: 15px;">
                  <div style="width: 100%; max-width: 650px; margin: 0 auto;">
                     <table width="100%" cellpadding="0" cellspacing="0" border="0" style="width: 100%; max-width: 650px; margin: 0 auto;">
                        <tbody>
                           <tr>
                              <td>
                                 <table width="100%" cellpadding="0" cellspacing="0" border="0" style="width: 100%; max-width: 600px; margin: 0 auto;">
                                    <tbody>
                                       <tr>
                                          <td height="40"></td>
                                       </tr>
                                       <tr>
                                          <td style="text-align: left;padding-right: 15px;padding-left: 15px;">
                                             <a href="https://www.clevermonks.com" target="_blank" style="display: inline-block;">
                                             <img src="https://bankopeny.com/wp-content/uploads/2022/02/Bankopeny.png" alt="" style="width: 150px;max-width: 100%;/* padding-left: 15px; *//* padding-right: 15px; */">
                                             </a>
                                          </td>
                                       </tr>
                                       <tr>
                                          <td height="25"></td>
                                       </tr>
                                       <tr>
                                          <td style="background-color: #f9f9f9;color: #000000;padding-top: 20px;padding-bottom: 20px;">
                                             <div style="color: #000000;font-size: 21px;text-align: left;font-family: Verdana, sans-serif;font-weight: bold;margin-bottom: -1px;">Client Profile (KCY)</div>
                                             <table style="
                                                width: 100%;
                                                " width="100%">
                                                <tbody>
                                                   <tr>
                                                      <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                         <table style="width: 100%;">
                                                            <tbody>
                                                               <tr>
                                                                  <th>
                                                                     <h5 style="color: #000000;font-size: 14px;text-align: left;margin-top: 15px;margin-bottom: 15px;">(1). Customer / Contracting Partner (please provide a passport copy and utility bill as well)</h5>
                                                                  </th>
                                                               </tr>
                                                               <tr>
                                                                  <td width="100%">
                                                                     <table style="width: 100%; border-spacing: 0px;">
                                                                        <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                           <tr>
                                                                              <td style="padding-top: 5px;padding-bottom: 5px;">
                                                                                 <table style="border-spacing: 0px; width: 100%;">
                                                                                    <tbody>
                                                                                       <tr>
                                                                                          <td width="25%">
                                                                                             <span style="/* display: flex; */">
                                                                                                <p style="
                                                                                                   ">Company Name:</p>
                                                                                             </span>
                                                                                          </td>
                                                                                          <td width="75%">
                                                                                             ${cfc_open_corporate_account_name_company ? `<span style="/* display: flex; */">
                                                                                                <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_name_company}`}</p>
                                                                                              </span>` : ''}
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td width="100%">
                                                                     <table style="width: 100%; border-spacing: 0px;">
                                                                        <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                           <tr>
                                                                              <td style="padding-top: 5px;padding-bottom: 5px;">
                                                                                 <table style="border-spacing: 0px; width: 100%;">
                                                                                    <tbody>
                                                                                       <tr>
                                                                                          <td width="25%" style="
                                                                                             vertical-align: top;
                                                                                             ">
                                                                                             <span style="/* display: flex; */">
                                                                                                <p style="
                                                                                                   ">Directors:</p>
                                                                                             </span>
                                                                                          </td>
                                                                                          <td width="75%" style="
                                                                                             vertical-align: top;
                                                                                             ">
                                                                                             ${cfc_open_corporate_account_directors_firstname ? `<span style="/* display: flex; */">
                                                                                                <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_firstname},
                                                                                                  ${cfc_open_corporate_account_directors_lastname},
                                                                                                  ${cfc_open_corporate_account_directors_nationality},
                                                                                                  ${cfc_open_corporate_account_directors_dob},
                                                                                                  ${cfc_open_corporate_account_directors_CityOfResidence},
                                                                                                  ${cfc_open_corporate_account_directors_Relation_each_other}`}</p>
                                                                                              </span>` : ''}
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td width="100%">
                                                                     <table style="width: 100%; border-spacing: 0px;">
                                                                        <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                           <tr>
                                                                              <td style="padding-top: 5px;padding-bottom: 5px;">
                                                                                 <table style="border-spacing: 0px; width: 100%;">
                                                                                    <tbody>
                                                                                       <tr>
                                                                                          <td width="25%" style="
                                                                                             vertical-align: top;
                                                                                             ">
                                                                                             <span style="/* display: flex; */">
                                                                                                <p style="
                                                                                                   ">Beneficial owners:</p>
                                                                                             </span>
                                                                                          </td>
                                                                                          <td width="75%">
                                                                                             ${cfc_open_corporate_account_beneficial_owners_firstname ? `<span style="/* display: flex; */">
                                                                                                <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_firstname},
                                                                                                  ${cfc_open_corporate_account_settlor_lastname},
                                                                                                  ${cfc_open_corporate_account_beneficial_owners_nationality},
                                                                                                  ${cfc_open_corporate_account_beneficial_owners_dob},
                                                                                                  ${cfc_open_corporate_account_beneficial_owners_CityOfResidence},
                                                                                                  ${cfc_open_corporate_account_beneficial_owners_Relation_each_other}`}</p>
                                                                                              </span>` : ''}
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td width="100%">
                                                                     <table style="width: 100%; border-spacing: 0px;">
                                                                        <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                           <tr>
                                                                              <td style="padding-top: 5px;padding-bottom: 5px;">
                                                                                 <table style="border-spacing: 0px; width: 100%;">
                                                                                    <tbody>
                                                                                       <tr>
                                                                                          <td width="25%">
                                                                                             <span style="/* display: flex; */">
                                                                                                <p style="
                                                                                                   ">Settlor:</p>
                                                                                             </span>
                                                                                          </td>
                                                                                          <td width="75%">
                                                                                             ${cfc_open_corporate_account_settlor_firstname ? `<span style="/* display: flex; */">
                                                                                                <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_firstname},
                                                                                                  ${cfc_open_corporate_account_settlor_lastname},
                                                                                                  ${cfc_open_corporate_account_settlor_nationality},
                                                                                                  ${cfc_open_corporate_account_settlor_dob},
                                                                                                  ${cfc_open_corporate_account_settlor_CityOfResidence},
                                                                                                  ${cfc_open_corporate_account_settlor_Relation_each_other}`}</p>
                                                                                              </span>` : ''}
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                            </tbody>
                                                         </table>
                                                      </td>
                                                   </tr>
                                                </tbody>
                                             </table>
                                          </td>
                                       </tr>
                                       <tr>
                                          <td>
                                             <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                                <tbody>
                                                   <tr>
                                                      <td width="600" style="">
                                                         <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                                            <tbody>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000;font-size: 16px;text-align: left;margin-top: 15px;margin-bottom: 15px;">Corporate account</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="24%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Name of company:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_name_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_name_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="24%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Jurisdiction of company:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_jurisdiction_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_jurisdiction_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Type of company:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${cfc_open_corporate_account_type_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_type_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000;font-size: 16px;text-align: left;margin-top: 15px;margin-bottom: 15px;">Beneficial owners</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">First Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_firstname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_firstname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Last Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_lastname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_lastname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Nationality:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_nationality ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_nationality}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Date of birth:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_dob ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_dob}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">country and city of residence:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_CityOfResidence ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_CityOfResidence}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Relation to each other:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_beneficial_owners_Relation_each_other ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_beneficial_owners_Relation_each_other}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000;font-size: 16px;text-align: left;margin-top: 15px;margin-bottom: 15px;">Directors</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">First Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_firstname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_firstname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Last Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_lastname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_lastname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Nationality:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_nationality ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_nationality}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Date of birth:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_dob ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_dob}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">country and city of residence:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_CityOfResidence ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_CityOfResidence}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Relation to each other:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_directors_Relation_each_other ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_directors_Relation_each_other}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000;font-size: 16px;text-align: left;margin-top: 15px;margin-bottom: 15px;">Settlor</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">First Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_settlor_firstname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_firstname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Last Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_settlor_lastname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_lastname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Nationality:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                        
                                                                                                         ${cfc_open_corporate_account_settlor_nationality ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_nationality}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Date of birth:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_settlor_dob ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_dob}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="23%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 10px;">Country and city of residence:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_settlor_CityOfResidence ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_CityOfResidence}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">Relation to each other:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_open_corporate_account_settlor_Relation_each_other ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_open_corporate_account_settlor_Relation_each_other}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (2). General Details
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Open an Account</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${investment_commercial_acc ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${investment_commercial_acc}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Open an Account</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${personal_corporate_acc ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${personal_corporate_acc}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Are you a signatory of the account</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${signatory_acc ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${signatory_acc}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Anybody else have signature right on the account</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${signature_right ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${signature_right}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Open the Bank account</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${bank_account_country ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${bank_account_country}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Prefer particular bank(s)</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${particular_bank ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${particular_bank}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (3). Personal Information
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">First Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${firstname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${firstname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Last Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${lastname ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${lastname}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Date of Birth:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${dob ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${dob}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Nationality:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${nationality ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${nationality}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">City Of Residence:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${CityOfResidence ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${CityOfResidence}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Address:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${address ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${address}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Email:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${email ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${email}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Contact Number</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${phone ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${phone}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (4).Educational Information
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Study</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${study ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${study}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Graduate Year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="75%">
                                                                                                         ${graduate ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${graduate}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Profession:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${profession ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${profession}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">From_year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${cfc_studies_from_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_studies_from_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">To year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${cfc_studies_to_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_studies_to_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Title</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="70%">
                                                                                                         ${cfc_studies_title ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_studies_title}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (5). Family Situation Alex Huber
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Marital Status</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="85%">
                                                                                                         ${MaritalStatus ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${MaritalStatus}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Partner</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">First Name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="85%">
                                                                                                         
                                                                                                         ${partner_spouse ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${partner_spouse}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                         
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Child</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Full Name with Age:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td>
                                                                                                         ${name_age_children ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${name_age_children}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Parents</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Father:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${father ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${father}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Mother:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${mother ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${mother}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Siblings</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Sibling:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td>
                                                                                                         ${siblings ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${siblings}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (6). Professional Background Alex Huber
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Occupational Status:</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">Name of actual Employer:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${actual_employer ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${actual_employer}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Your title:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="">
                                                                                                         ${employer_title ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${employer_title}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">Nature of your business:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${business_nature ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${business_nature}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="line-height: 22px;">Business headquarter:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="">
                                                                                                         ${business_headquarter_country ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${business_headquarter_country}`}</p>
                                                                                                          </span>` : ''}
                                                                                                          ${business_headquarter_city ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${business_headquarter_city}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">Employment Start Date:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${employment_start_date ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${employment_start_date}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="line-height: 22px;">Website:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="">
                                                                                                         ${company_website ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${company_website}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Shareholder:</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Turnover p.a:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${approximate_turnover_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${approximate_turnover_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Number of employees:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="20%">
                                                                                                         ${approx_employees ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${approx_employees}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Share in the company:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                        
                                                                                                         ${share_of_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${share_of_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Approx value of the company:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="20%">
                                                                                                         ${approx_value_of_company ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${approx_value_of_company}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">Previous Occupations:</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Work before:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="85%">
                                                                                                         ${worked_before ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${worked_before}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">From year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${before_from_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${before_from_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">To year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${before_to_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${before_to_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Company name:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${before_company_name ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${before_company_name}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Role:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${before_work_title ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${before_work_title}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%">
                                                                                                         <p style="padding-right: 10px;">Compensation/year:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${before_work_compensation_per_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${before_work_compensation_per_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="20%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="line-height: 22px;">PEP (political exposed person):</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="">
                                                                                                         ${political_exposed_person ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${political_exposed_person}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">(7).Joint signatory:</h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="40%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">People will have signatory rights:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="60%">
                                                                                                         ${cfc_signature_right_people ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_signature_right_people}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="22%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Name of signatory:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${cfc_signature_right_name ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_signature_right_name}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="30%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Relation to Joint signatory:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="20%">
                                                                                                         ${cfc_signature_right_relation ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_signature_right_relation}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    Signatories beneficial owners:
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 10px;">Name of signatories beneficial owners:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${cfc_signatories_beneficial_owners_name ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_signatories_beneficial_owners_name}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="27%" style="padding-right: 9px; vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 2px;">Relation to signatories beneficial owners:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="24$" style="vertical-align: top;">
                                                                                                         ${cfc_signatories_beneficial_owners_relation ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${cfc_signatories_beneficial_owners_relation}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (8).Financial Information:
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="25%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="line-height: 22px; /* margin-top: 0px; */">Avg Annual Income:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${amount_in_usd ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${amount_in_usd}`}</p>
                                                                                                          </span>` : ''}
                                                                                                          ${amount_in_usd_source ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${amount_in_usd_source}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">financial worth in USD:</p>
                                                                                                      </td>
                                                                                                      <td width="24%">
                                                                                                         ${financial_worth_in_usd ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${financial_worth_in_usd}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                                 <table style="width: 100%;">
                                                                                    <tbody>
                                                                                       <tr>
                                                                                          <th>
                                                                                             <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                                Real estates :
                                                                                             </h5>
                                                                                          </th>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td width="100%">
                                                                                             <table style="width: 100%; border-spacing: 0px;">
                                                                                                <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                                   <tr>
                                                                                                      <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                                         <table style="border-spacing: 0px; width: 100%;">
                                                                                                            <tbody>
                                                                                                               <tr>
                                                                                                                  <td width="24%" style="vertical-align: top;">
                                                                                                                     <span style="/* display: flex; */">
                                                                                                                        <p style="padding-right: 10px;">Types:</p>
                                                                                                                     </span>
                                                                                                                  </td>
                                                                                                                  <td width="24%" style="vertical-align: top;">
                                                                                                                     ${assets_type ? `<span style="/* display: flex; */">
                                                                                                                        <p style="/* border-bottom: 1px solid; */">${`${assets_type}`}</p>
                                                                                                                      </span>` : ''}
                                                                                                                  </td>
                                                                                                                  <td width="4%"></td>
                                                                                                                  <td width="27%" style="padding-right: 9px; vertical-align: top;">
                                                                                                                     <span style="/* display: flex; */">
                                                                                                                        <p style="padding-right: 2px;">country:</p>
                                                                                                                     </span>
                                                                                                                  </td>
                                                                                                                  <td width="24$" style="vertical-align: top;">
                                                                                                                     ${assets_country ? `<span style="/* display: flex; */">
                                                                                                                        <p style="/* border-bottom: 1px solid; */">${`${assets_country}`}</p>
                                                                                                                      </span>` : ''}
                                                                                                                  </td>
                                                                                                               </tr>
                                                                                                            </tbody>
                                                                                                         </table>
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                   <tr>
                                                                                                      <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                                         <table style="border-spacing: 0px; width: 100%;">
                                                                                                            <tbody>
                                                                                                               <tr>
                                                                                                                  <td width="24%" style="vertical-align: top;">
                                                                                                                     <span style="/* display: flex; */">
                                                                                                                        <p style="padding-right: 10px;">city:</p>
                                                                                                                     </span>
                                                                                                                  </td>
                                                                                                                  <td width="24%" style="vertical-align: top;">
                                                                                                                     ${assets_city ? `<span style="/* display: flex; */">
                                                                                                                        <p style="/* border-bottom: 1px solid; */">${`${assets_city}`}</p>
                                                                                                                      </span>` : ''}
                                                                                                                  </td>
                                                                                                                  <td width="4%"></td>
                                                                                                                  <td width="27%" style="padding-right: 9px; vertical-align: top;">
                                                                                                                     <span style="/* display: flex; */">
                                                                                                                        <p style="padding-right: 2px;">Approximate value in USD:</p>
                                                                                                                     </span>
                                                                                                                  </td>
                                                                                                                  <td width="24$" style="vertical-align: top;">
                                                                                                                     ${assets_approx_value ? `<span style="/* display: flex; */">
                                                                                                                        <p style="/* border-bottom: 1px solid; */">${`${assets_approx_value}`}</p>
                                                                                                                      </span>` : ''}
                                                                                                                  </td>
                                                                                                               </tr>
                                                                                                            </tbody>
                                                                                                         </table>
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <th>
                                                                                             <h5 style="color: #000000; font-size: 16px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                                bankable assets:
                                                                                             </h5>
                                                                                          </th>
                                                                                       </tr>
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">Name of bank:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${name_of_bank ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${name_of_bank}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Aprox. Amount in USD:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${bankable_assets_approx_amount ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${bankable_assets_approx_amount}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">source of your wealth:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${source_of_wealth ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${source_of_wealth}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">source of your wealth amount in USD:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${source_of_wealth_amount ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${source_of_wealth_amount}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (9).Investment Objectives and Risk Profile:
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">Investment Objective:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${investment_objective ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${investment_objective}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Risk Tolerance:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${risk_tolerance ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${risk_tolerance}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">Investment Experience:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${investment_experience ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${investment_experience}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Investment Knowledge:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${investment_knowledge ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${investment_knowledge}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (10).Account Details:
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="28%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 10px;">Amount to transfer to the new bank account:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="72%">
                                                                                                         ${approx_amount_transfer_new_bank_account ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${approx_amount_transfer_new_bank_account}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">name of bank:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${send_fund_from_bank_name ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${send_fund_from_bank_name}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">Country:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${send_fund_from_bank_country ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${send_fund_from_bank_country}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="20%" style="vertical-align: top;">
                                                                                                         <p style="padding-right: 10px;">transfers you intend to do every year:</p>
                                                                                                      </td>
                                                                                                      <td width="24%" style="vertical-align: top;">
                                                                                                         ${transfers_intend_to_do_every_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${transfers_intend_to_do_every_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                      <td width="4%"></td>
                                                                                                      <td width="15%" style="vertical-align: top;">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="">transfers you intend to receive every year:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="" style="vertical-align: top;">
                                                                                                         ${transfers_intend_to_receive_every_year ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${transfers_intend_to_receive_every_year}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                               <tr>
                                                                  <td style="/* padding-left: 30px; */ /* padding-right: 30px; */">
                                                                     <table style="width: 100%;">
                                                                        <tbody>
                                                                           <tr>
                                                                              <th>
                                                                                 <h5 style="color: #000000; font-size: 18px; text-align: left; margin-top: 15px; margin-bottom: 15px;">
                                                                                    (11).Conformation:
                                                                                 </h5>
                                                                              </th>
                                                                           </tr>
                                                                           <tr>
                                                                              <td width="100%">
                                                                                 <table style="width: 100%; border-spacing: 0px;">
                                                                                    <tbody style="font-size: 14px; vertical-align: inherit;">
                                                                                       <tr>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="28%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 10px;">I confirm that the information provided:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="72%">
                                                                                                         ${confirm_information_provided ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${confirm_information_provided}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                          <td style="padding-top: 10px;padding-bottom: 10px;">
                                                                                             <table style="border-spacing: 0px; width: 100%;">
                                                                                                <tbody>
                                                                                                   <tr>
                                                                                                      <td width="28%">
                                                                                                         <span style="/* display: flex; */">
                                                                                                            <p style="padding-right: 10px;">I understand that the information provided for KYC Purposes:</p>
                                                                                                         </span>
                                                                                                      </td>
                                                                                                      <td width="72%">
                                                                                                         ${understand_information_provided ? `<span style="/* display: flex; */">
                                                                                                            <p style="/* border-bottom: 1px solid; */">${`${understand_information_provided}`}</p>
                                                                                                          </span>` : ''}
                                                                                                      </td>
                                                                                                   </tr>
                                                                                                </tbody>
                                                                                             </table>
                                                                                          </td>
                                                                                       </tr>
                                                                                    </tbody>
                                                                                 </table>
                                                                              </td>
                                                                           </tr>
                                                                          
                                                                        </tbody>
                                                                     </table>
                                                                  </td>
                                                               </tr>
                                                            </tbody>
                                                         </table>
                                                      </td>
                                                   </tr>
                                                </tbody>
                                             </table>
                                          </td>
                                       </tr>
                                    </tbody>
                                 </table>
                              </td>
                           </tr>
                        </tbody>
                     </table>
                  </div>
               </td>
            </tr>
         </tbody>
      </table>
   </body>
</html>
  `

}